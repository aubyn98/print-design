...
export type Palette = Omit<CanvasConfigs, 'ratio'> & { menu: typeof DEFAULTMENU }
...
export interface shadowH {
  x: number
  width: number
}
export interface shadowV {
  y: number
  height: number
}
export const drawHorizontalRuler = (ctx: CanvasRenderingContext2D, start: number, shadow: shadowH, options: Options) => {
...
export const drawVerticalRuler = (ctx: CanvasRenderingContext2D, start: number, shadow: shadowV, options: Options) => {
  const { scale, width, height, canvasConfigs } = options
  const { bgColor, fontColor, shadowColor, ratio, longfgColor, shortfgColor } = canvasConfigs
  ...
  for (let value = startValue_10, count = 0; value < endValue; value += gridSize_10, count++) {
    ...
    ctx.fillText(String(value), 4 * ratio, 7 * ratio) // 绘制文字
    // 回复刚刚保存的状态
    ctx.restore()
